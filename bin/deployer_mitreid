...
#    - access_token
#    - deployer_name
def update_data(messages, issuer_url, access_token, perun_client_api=None):
    """
    ...
    def deploy_func(msg):
        return call_mitreid(msg, mitreid_agent)
...
if __name__ == "__main__":
    ...
    if "perun" in config["mitreid"]:
        perun_client_api = PerunClientApi(config["mitreid"]["perun"])
    ...
    while True:
        ...
        if len(messages) > 0:
            ...
            ams.ack(ids)
            responses = update_data(messages, config["mitreid"]["issuer"], access_token, perun_client_api)
