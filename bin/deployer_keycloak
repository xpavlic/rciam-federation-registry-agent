...
from Utils.common import (
    get_keycloak_issuer,
    get_log_conf,
    publish_ams, process_data_generic,
)
...
def process_data(messages, access_token, keycloak_config, perun_client_api=None):
    """
    Updated by Jan Pavlíček (xpavli95@stud.fit.vutbr.cz) to use generic data processing function
    """
    keycloak_agent = KeycloakClientApi(keycloak_config["auth_server"], keycloak_config["realm"], access_token)

    def deploy_func(msg):
        return deploy_to_keycloak(msg, keycloak_agent, keycloak_config)
...
if __name__ == "__main__":
    ...
    if "perun" in config["keycloak"]:
        perun_client_api = PerunClientApi(config["keycloak"]["perun"])
    ...
    while True:
        ...
        if len(messages) > 0:
            ...
            ams.ack(ids)
            responses = process_data(messages, access_token, config["keycloak"], perun_client_api)
