...
from Utils.common import create_ams_response, get_log_conf, deploy_to_perun
...
def publish_ams(ams_agent, response, messages, deployer_name, proxy_deploy_success):
    log.info("Publish message to ams")
    pub_messages = []
    for message in messages:
        try:
            msg = create_ams_response(
                response, message["id"], deployer_name, "", "", proxy_deploy_success
            )
        except:
...
if __name__ == "__main__":
    """
    Updated by Jan Pavlíček (xpavli95@stud.fit.vutbr.cz) to support service deployment to Perun IDM
    """
    ...
    if "perun" in config["ssp"]:
        perun_client_api = PerunClientApi(config["ssp"]["perun"])
        ...
        if len(messages) > 0:
            ...
            if response["status"] in [200, 201, 204]:
                ...
                if perun_client_api:
                    for msg in messages:
                        try:
                            if msg.get("deployment_type") and msg.get("protocol") == "saml":
                                deploy_success = deploy_to_perun(deepcopy(msg), perun_client_api)
                                if not deploy_success:
                                    response["status"] = 0
                                    response["error"] = "Deployment to Perun failed."
                        except Exception as e:
                            log.critical(f"Perun deployment exception: {e}")
                            response = {"status": 0, "error": "Deployment to Perun failed."}

            log.info("Message received from SSP: " + str(response))
            publish_ams(ams, response, messages, config["ssp"]["ams"]["deployer_name"], proxy_deploy_success)
