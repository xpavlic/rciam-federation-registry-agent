...
class PerunClientApi(object):
    ...
    def delete_admins_group(self, facility_id):
        try:
            ...
            if admins_group_id:
                ...
                if not result:
                    log.warning(f"Could not delete group {admins_group_id}")
            else:
                log.info("No admins group ID found for facility: " + facility_id)
    ...
    def register_new_service_in_perun(self, perun_message):
        ...
        try:
            ...
            if not facility or not facility.get("id") or not facility.get(
                ...
                log.critical("Creating facility in Perun has failed")
                return False
        ...
        try:
            ...
            log.critical(f"Caught an exception when registering service to Perun: {e}")
            if admins_group_id is not None:
                try:
                    self.perun_rpc_adapter.delete_group(admins_group_id)
                except Exception as e_group:
                    log.critical(f"Caught an exception when removing admin group: {e_group}")
            try:
                self.perun_rpc_adapter.delete_facility(facility_id)
            except Exception as e_facility:
                log.critical(f"Caught an exception when removing facility: {e_facility}")
            return False
        return True
    ...
    def update_service_in_perun(self, perun_message):
        try:
            ...
            if not facilities:
                log.critical("No facility found in Perun for service id: " + perun_message["id"])
                return False
            ...
            if facility["name"] != perun_message["service_name"] or (
                ...
                if not self.perun_rpc_adapter.update_facility(facility):
                    log.critical("Updating facility name or description failed for facility with id: " + facility_id)
                    return False
            perun_attributes = self.build_perun_attributes(perun_message)
            if not self.perun_rpc_adapter.set_facility_attributes(
                    facility_id,
                ...
                log.critical("Updating facility attributes failed for facility with id: " + facility_id)
                return False
        except Exception as e:
            log.critical(f"Caught an exception when updating service in Perun: {e}")
            return False
        return True
    ...
    def delete_service_in_perun(self, perun_message):
        try:
            ...
            log.critical(f"Caught an exception when deleting service: {e}")
            return False
        return True
